...
export const MobileLayout = ({ children }: MobileLayoutProps) => {
  ...
  const navItems = [
    ...
    { path: "/ban-giao-pda", label: "Bàn giao PDA", icon: TabletSmartphone },
  ];
      ...
      <div className="drawer-content flex min-h-screen min-w-0 flex-col relative z-10">
        <header className="navbar h-14 min-h-14 bg-base-100/90 backdrop-blur-md border-b border-base-200 sticky top-0 z-40 w-full px-2 sm:px-3 md:px-6">
          <div className="flex-none">
            <button type="button" onClick={toggleDrawer} aria-label="open sidebar" className="btn btn-square btn-ghost drawer-button relative z-50 min-h-11 min-w-11 touch-manipulation rounded-xl p-2">
          ...
          <div className="mx-1 min-w-0 flex-1 px-1 sm:mx-2 sm:px-2 font-black text-base sm:text-lg md:text-xl tracking-tight flex items-center gap-2">
            <span className="p-1.5 bg-primary/10 text-primary rounded-xl"><Package className="w-5 h-5" /></span>
            <span className="truncate" title={currentSoc}><span className="text-primary font-black">SPX</span> <span className="align-middle">{currentSoc}</span></span>
          </div>

          <div className="flex-none flex items-center gap-2"><ToggleTheme /></div>
        </header>
      ...
      <div className="drawer-side z-50">
        ...
        <div className="menu bg-base-100 text-base-content min-h-full w-[min(86vw,20rem)] p-4 pb-[max(1rem,env(safe-area-inset-bottom))] sm:p-5 flex flex-col justify-between shadow-2xl">
          <div className="space-y-5">
            <div className="pb-4 border-b border-base-200 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-primary to-secondary flex items-center justify-center text-primary-content font-bold shadow-md"><Package className="w-5 h-5" /></div>
                <div><span className="font-extrabold text-base block leading-tight">Ops FTE</span><span className="text-xs text-base-content/60 font-medium">{currentSoc}</span></div>
              </div>
              <label htmlFor="mobile-sidebar-drawer" className="btn btn-sm btn-circle btn-ghost min-h-11 min-w-11"><X className="w-4 h-4" /></label>
            </div>
            ...
            <ul className="space-y-1.5 text-sm font-semibold">
              {navItems.map((item) => {
                ...
                return (
                  <li key={item.path}>
                    <Link to={item.path} onClick={closeDrawer} className={`flex min-h-11 items-center gap-3 px-4 py-3 rounded-xl transition-all ${isActive ? "bg-primary text-primary-content font-bold shadow-xs" : "hover:bg-base-200 text-base-content/80"}`}>
          ...
          <div className="border-t border-base-200 pt-4">
            <Link to="/cai-dat" onClick={closeDrawer} className={`flex min-h-11 items-center gap-3 px-4 py-3 rounded-xl transition-all ${location.pathname === "/cai-dat" ? "bg-primary text-primary-content font-bold" : "hover:bg-base-200 text-base-content/80 font-medium"}`}>
